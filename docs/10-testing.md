...
- **`stream_chat(...)`** — POST to the global-chat stream endpoint and consume SSE by hand, returning a dict with `query_plan`, `search_count`, `assistant_text`, `study_ids_mentioned`, `result_study_ids`, `ui_payload`, `step_done_labels`, `pinned_studies`, and `tool_ui_payloads`. It parses **both** the legacy events (`query_plan`, `step_done`) and the agentic ones (`segment_tool_result`), so one helper covers both chat paths. Study IDs are recovered from the assistant's prose with a `\b(\d{4,6})\b` regex — deliberately loose, used only as a fallback signal alongside the structural checks.
...
`chat_search_ids(backend_url, query_plan)` closes the loop: it takes the chat's own planner output and re-runs it through `/api/search`, which is how test 2.3 compares the two paths using one endpoint.
...
- **No test for the one-search invariant.** `test_agent_tool_call.py` asserts `_execute_tool_call` returns the `is_search_studies` flag, but nothing asserts the consequence — that `search_studies` is *absent from `active_tools`* on the next iteration ([`05-agent.md`](05-agent.md)). The flag is tested; the schema mutation it drives is not.
...
- **Project chat has no dedicated tests.** `chat_routes.py`'s streaming path is covered only incidentally.
