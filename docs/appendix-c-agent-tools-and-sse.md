...
`search_studies` and `search_project_studies` may run up to `SEARCH_CALLS_PER_MESSAGE` (default 5) times per user message. Both provider loops track a local `search_calls_used` counter, incremented when `_execute_tool_call` returns `consumed_search_slot=True` — which it does only for an *executed* search: an empty-input early return (`ToolResult.executed=False`) or a crashing search does not consume a slot. `_execute_tool_call` returns the triple `(result_text, consumed_search_slot, failed)`; `failed` is the structured fact that the tool raised, which the loops use for the fallback text instead of sniffing `result_text`. Once the counter reaches the cap, `_tools_within_search_budget` strips both budgeted search tools from the schema handed to the model on every subsequent round (both wire shapes — OpenAI `{"function": {"name": …}}` and Anthropic `{"name": …}`), and a further attempted call short-circuits with a "synthesize from the results you have" message without executing.

This is enforced at the schema level, not just by the tool description — past the cap, the model cannot see the search tools as options at all. The other tools remain available on every iteration up to `max_iters` (default `7` in `stream_agent`). [`05-agent.md`](05-agent.md) covers the design rationale (schema mutation over prompt instruction); this appendix covers only the mechanics.
...
Two events, documented together because they usually pair. Emitted by context-prep steps (`build_context`, `deep_context`, `pinned_reports` in both routes), by the shared pin/report flows (`pin_flow.py :: stream_pin_flow`, `request_utils.py :: stream_samples_report`), and — since 2026-08-31, forwarded by `chat_turn.py :: stream_chat_turn` — from inside the agent turn itself: `llm_retry.py` emits a paired `retry` step around each backoff, and both agent loops emit an **unpaired** `step_start` named `synthesis` ("Tool-round limit (N) reached — writing final answer…") when the round budget is exhausted on tool calls; the `done` event closes it (`_finalizeStreamedMessage` clears `pendingStep`). Payload for `step_start`: `{"name": str, "label": str}`. Payload for `step_done`: `{"name": str, "label": str, "detail": str}` (detail sometimes omitted). Client: `onStepStart` sets `m.pendingStep`; `onStepDone` appends to `m.steps` — regardless of mode, and `AgentMessageBubble` renders both above the segments, so agent turns show steps too. Steps are session-only: they are never persisted into `ui_payload`, so they vanish on reload (TKT-079).
...
Terminal event for a successful turn, on both paths. Payload always includes `{"chat_id": str, "persisted": true, "title": str}` — `title` is the chat's current stored title, read fresh from the database after `helpers/chat_turn.py :: _done_payload` joins the turn's background title-generation thread (`helpers/chat_title.py`, capped at 5s); it is the provisional first-60-chars title if the LLM hasn't finished (or wasn't needed), the LLM-generated title if it has, or a rename that landed mid-turn. Global chat additionally includes `"pinned_studies": [...]` whenever the turn touched pins — either the pin-flow's `all_pinned` result, or (for agent turns specifically) a fresh `list_pinned_studies(chat_id, SCOPE_GLOBAL)` re-read after the fact, because `get_study_report` and `pin_study` may have pinned studies mid-turn without any other event reporting the updated list. Client: `onDone` calls `applyStreamDone(chatId, payload)`, applying `payload.title` unconditionally (the server value can never be behind a rename) — this is also where segments get frozen into `m.ui`; see below.
...
- `step_start`/`step_done` pairs from context prep, pin/report flows, and `llm_retry` nest correctly — none of those emitters issues a second `step_start` before closing the first. The one deliberate exception is the agent loop's `synthesis` `step_start`, which has no `step_done`: it is a progress notice for the forced-synthesis call and is closed by `done`.
...
*See also: [`05-agent.md`](05-agent.md) for how the model is prompted to fill tool arguments and choose between dimensions, and the design rationale behind the search budget and forced synthesis · [`06-streaming-and-chat.md`](06-streaming-and-chat.md) for the full SSE-to-hydration round trip, the dual-authoring hazard, and the segment tri-state · [`04-search.md`](04-search.md) for how `search_studies` and `search_by_sample` bound their underlying queries · [`appendix-d-configuration.md`](appendix-d-configuration.md) for `MODEL_METADATA`, `model_supports_tools`, and the sample-search candidate caps.*