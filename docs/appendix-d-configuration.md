...
- **The only variables that legitimately live in the shell are the ones the start scripts export themselves** — `QIITA_CONFIG_FP` and `QIITA_EXPERIMENT_DB_PATH` in `start_barnacle.sh`, which also re-exports a set of keys read out of `.env` (`PI_SIDECAR_SECRET`, `PI_SIDECAR_PORT`, `PI_SIDECAR_HOST`, `PI_NODE_BIN`, `API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `QIITA_EXPLORE_PORT`) — the first seven so the Node sidecar, which cannot read `.env` itself, can see them, and `QIITA_EXPLORE_PORT` so the script's own `PORT` can come from `.env` like everything else. Everything else belongs in the `.env` file. (A shell export of a `.env` name still wins, because `load_dotenv()` does not override variables already present in the environment — but relying on that is how configuration drift starts.)
  - `QIITA_EXPLORE_PORT` was a shell-only variable until recently, and the transition had teeth: `PORT="${QIITA_EXPLORE_PORT:-5001}"` sat ~50 lines *above* the `.env` parse, so setting it in `.env` was silently ignored and the backend bound 5001 while an SSH tunnel expected 5002. The assignment now lives below the parse. If you add another `.env`-sourced key that the shell script itself consumes, check the same ordering.
...
| `QIITA_EXPLORE_PORT` | `5001` | `start_barnacle.sh` (shell, not `config.py`) | Port gunicorn binds. Settable from `.env` — it is in the script's `wanted` tuple and `PORT` is resolved after that parse. `FLASK_INTERNAL_URL` derives from it, so the sidecar follows automatically. Must agree with whatever the frontend dials (`frontend/index.html`'s `api-base`, currently `:5002` for dev) and with any SSH tunnel's far end. | Yes |
...
The sidecar itself is a separate Node process, started independently (`qiita_explore/pi_sidecar/start_sidecar.sh`), with its own env vars — `PI_SIDECAR_PORT` (default `5100`), `PI_SIDECAR_HOST` (default `127.0.0.1`), `FLASK_INTERNAL_URL` (default `http://127.0.0.1:5001`, but `start_barnacle.sh` overrides it to the actual `$PORT`), `PI_SIDECAR_STATE_DIR` (default `pi_sidecar/.state`), plus `OPENAI_API_KEY`/`API_KEY` (reused for the NRP provider) and `ANTHROPIC_API_KEY` (picked up by pi's built-in Anthropic provider). It refuses to start without `PI_SIDECAR_SECRET`, without `node_modules`, on a Node older than major 22, or — the authoritative check — if `import("@earendil-works/pi-coding-agent")` throws under the selected interpreter. `start_sidecar.sh --check` runs all of that and exits without serving; `start_barnacle.sh` calls it before spawning gunicorn.
