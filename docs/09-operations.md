...
### Starting the pi sidecar

The sidecar is never optional — chat has no other runtime to fall back to (see [`05-agent.md`](05-agent.md)), so `start_barnacle.sh` always starts it, and `run.py` refuses to boot at all without a working pi configuration (`config.pi_config_errors()` checks `PI_SIDECAR_SECRET` and `PI_SCOPE_TOKEN_KEY`). Start it **before or alongside** gunicorn if starting components by hand — Flask calls out to it synchronously mid-request, so a chat turn arriving before the sidecar is up fails with a connection error surfaced through the normal `error` SSE event, not a crash.
...
`start_barnacle.sh` starts it: pi config is resolved and validated first, then **the sidecar is pre-flighted**, then gunicorn goes up, the script waits for it to answer, then it launches the sidecar (which fetches its model roster from Flask at boot). Ctrl-C stops both, and if either dies the other is torn down. There is still no process manager wiring the pair into the OS (this repo has none for gunicorn either; see the note on `systemctl` above).

**The pre-flight exists because the sidecar starts last but can fail for reasons knowable first.** `start_barnacle.sh` runs `pi_sidecar/start_sidecar.sh --check` — every validation the sidecar does at startup (secret, Node, `node_modules`, pi imports), minus actually serving — *before* spawning gunicorn. Without it the failure ordering is genuinely misleading: gunicorn logs `Listening at: http://0.0.0.0:5002`, the readiness probe passes, the sidecar then dies at import, and the die-together loop tears down a Flask that was healthy. The observable result is a backend that reports a clean boot and is gone nine seconds later, with the real cause a Node stack trace scrolled off the top. Pre-flighting turns that into one message and no gunicorn.

**A missing secret is a hard failure at startup, not a warning.** `PI_SIDECAR_SECRET`/`PI_SCOPE_TOKEN_KEY` used to be optional, guarding a feature-flagged rollout; a missing value used to print *"pi sidecar not started (legacy chat path only)"* and carry on. That fallback — and the flags that gated it (`PI_BACKEND_GLOBAL`/`PI_BACKEND_PROJECT`) — no longer exist. `start_barnacle.sh` checks for `PI_SIDECAR_SECRET` and exits with a clear message before even trying to start gunicorn; `run.py` checks again via `config.pi_config_errors()` and refuses to boot if either value is missing, so a misconfigured host fails loudly at the earliest possible point rather than serving a UI that 401s on every chat turn.
...
**Requires Node ≥ 22.19 (pi's declared floor).** This page has now been wrong about this twice, in the same direction, so the specifics matter. First it called pi's declared `>=22.19.0` a support-policy floor with no technical teeth; that was wrong, because the undici bundled in pi's tree dies at import with `TypeError: webidl.util.markAsUncloneable is not a function` — a stack trace naming neither pi nor the Node version. The correction was a **major-version** floor (`>= 22`), and that was wrong too: barnacle ran **22.6.0**, satisfied `major >= 22`, and died at that exact import. 22.12.0 is verified working; 22.6.0 is not; the true breaking point is somewhere between.

Rather than guess again at where in the 22.x range it sits, `start_sidecar.sh` no longer relies on a version comparison as its gate. It keeps a cheap `major >= 22` check for a fast message on obviously-wrong interpreters, then **actually imports pi** (`node --input-type=module -e 'await import("@earendil-works/pi-coding-agent")'`, ~500ms) and refuses to start if that throws — reporting the running version, pi's declared `engines.node` read from the installed package, and the underlying error. That tests the thing that breaks instead of a proxy for it, and it catches the next import-time incompatibility even when it isn't a Node version at all.
...
Note `PI_NODE_BIN` is not a guarantee of a *new enough* interpreter — barnacle had it set to a hand-installed `~/opt/node22/bin/node` that was 22.6.0. Pinning the path only fixes *which* Node runs; the import check above is what establishes it can run pi. A Node upgrade **within** the same major (22.6 → 22.19) needs no `npm ci` — `.pi-node-major` still reads `22` and the installed tree stays ABI-valid.
...
**pi sidecar session files.** Every chat has one JSONL session file under `pi_sidecar/.state/sessions/<user_id>/` — **per user**, since the layout change described below. Deleting a chat calls `POST /session/delete` on the sidecar as a best-effort cleanup (`backend/helpers/pi_client.py :: delete_session`), which removes it — but that call is skipped entirely if `PI_SIDECAR_SECRET` is unset (pi not configured at all), and it's fire-and-log-only if the sidecar is unreachable at delete time. There is no reaper for orphaned session files independent of chat deletion, mirroring the pattern above: periodic pruning against SQLite's chat tables (a session file with no matching `chat_id` in either `project_chats` or `global_chats` is safe to remove) is the workaround until one exists.
