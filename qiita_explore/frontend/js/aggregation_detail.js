...
// open its metadata), and the metadata pane itself. Loads before
// aggregations.js, whose AggregationsTab renders these.
...
function AggregationDetail({ a, agg, picked, onPickSample }) {
  ...
  // Plain link like the per-artifact manifest (fastq_manifest.js): the session
  // cookie rides along on top-level navigation.
    ...
    <div className="agg-detail">
      <div className="agg-detail-head">
        ...
        {totalSelected > 0
          ...
          : <span className="ao-file-btn agg-disabled" title="Check at least one sample">↓ Download sample CSV</span>}
      </div>
      {studies.length === 0
...
// merged in server-side. Load-more shape from merge_detail.js StudySampleTable.
function AggregationSampleTable({ a, agg, study, picked, onPickSample }) {
  ...
  const load = async (offset, query, append) => {
    const mine = ++seq.current;   // a slower earlier page must not overwrite a newer one
    setLoading(true); setErr('');
    try {
      const d = await apiJson(
        `/aggregations/${aid}/studies/${sid}/samples?offset=${offset}&limit=${_AGG_PAGE}&q=${encodeURIComponent(query)}`);
  ...
  // First page on mount; a filter change reloads page 0 after 300 ms of quiet
  // (the filter is an ILIKE over the whole sample table — ~1 s on 40k rows).
  useEffect(() => {
    const t = setTimeout(() => load(0, q.trim(), false), q ? 300 : 0);
  ...
  const bulk = async (body) => {
    setErr('');
    try { await agg.setSamples(aid, sid, body); await load(0, q.trim(), false); }
    ...
    <div className="agg-samples-panel">
      ...
      <div className="agg-samples-wrap">
        <table className="prep-table">
          <thead>
            <tr><th></th><th>Sample ID</th>{columns.map(c => <th key={c}>{c}</th>)}</tr>
          </thead>
          <tbody>
            {rows.map(r => (
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={columns.length + 2} className="agg-samples-empty">No samples match.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      <div className="agg-samples-foot">
        ...
        {!loading && hasMore && (
          <button className="merge-btn-ghost" onClick={() => load(rows.length, q.trim(), true)}>
            Load more ({rows.length.toLocaleString()} of {total.toLocaleString()})
          </button>
        )}
        {!loading && !hasMore && rows.length > 0 && (
          <span className="agg-samples-msg">{total.toLocaleString()} {q.trim() ? 'matching ' : ''}samples</span>
        )}
      </div>
    </div>
  );
