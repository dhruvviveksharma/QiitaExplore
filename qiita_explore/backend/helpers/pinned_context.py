...
def _build_full_samples_block(study_id: int, budget_chars: int, *, tools_available: bool = True):
    ...
    if truncated_at is not None:
        hatch = f"; call get_study_report({study_id}) for more" if tools_available else ""
...
def _pinned_manifest_line(study_id: int, header: dict) -> str:
...
def _build_pinned_reports_context(study_ids, model: str, *, tools_available: bool = True):
    ...
    with ThreadPoolExecutor(max_workers=min(len(inline), 4)) as pool:
        blocks = list(pool.map(
            lambda sid: _build_full_samples_block(sid, per_study, tools_available=tools_available),
            inline,
        ))
        # Resolve overflow headers in the same fan-out; done after the `with`
        # these serialize into real Postgres round-trips on a cold worker.
    ...
    if overflow:
        text += (
            ...
            + "\n".join(_pinned_manifest_line(sid, h) for sid, h in zip(overflow, headers))
        )
    return text
